...
class _AITextPromptScreenState extends State<AITextPromptScreen> {
  ...
  Future<void> _checkBackendHealth() async {
    try {
      ...
      final response = await http.get(
        Uri.parse("http://192.168.34.234:7000/health"),
        timeout: const Duration(seconds: 5),
      );
      setState(() {
        _backendHealthy = response.statusCode == 200;
      });
      print('✅ [FRONTEND] Backend: ${_backendHealthy ? "CONNECTED" : "OFFLINE"}');
    } catch (e) {
      print('❌ [FRONTEND] Backend error: $e');
      setState(() {
  ...
  Future<void> _generateDesign() async {
    ...
    try {
      print('🚀 [FRONTEND] Generating design for: $prompt');

      final response = await http.post(
        Uri.parse("http://192.168.34.234:7000/generate"),
      ...
      if (response.statusCode == 200) {
        ...
        if (data["success"] == true && data["image_base64"] != null) {
          ...
          print('✅ [FRONTEND] Image received and displayed!');
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
