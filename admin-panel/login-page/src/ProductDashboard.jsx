    ...
    <div className="pd-container">
      ...
      <div className="pd-controls-bar">
        <div className="pd-controls-left">
          ...
          <button
            className={`pd-view-btn ${viewMode === "grid" ? "active" : ""}`}
          ...
          <div className="pd-filter-group">
            {["All Categories", "In Stock", "Pending", "Flagged"].map((f) => (
              <button
                key={f}
                className={`pd-filter-btn ${selectedFilter === f ? "active" : ""}`}
          ...
          <tbody>
            {products.map((product) => (
              <tr key={product.id}>
                ...
                <td
                  className={`pd-stock ${product.stock === 0 ? "out-of-stock" : ""}`}
                >
                  {product.stock === 0 ? "Out of Stock" : product.stock}
                </td>
                <td>
                  <span
                    className={`pd-status-badge ${getStatusColor(product.status)}`}
                  >
                    {product.status}
                  </span>
                </td>
